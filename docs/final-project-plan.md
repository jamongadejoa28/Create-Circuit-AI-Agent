...
**갱신 (2026-08-08, Codex 계층 분류 채택)**: 지식 편입 기준을 "공식/ERC 규칙 중심"에서 Codex의 5계층(Tier 1 실행 규칙 / 2 회로 패턴 / 3 선택 근거·실패 모드 / 4 검수 예제 / 5 원문 근거)으로 확장했다. 1차 확장 결과 34→96 엔트리(circuit_pattern 21, device_rule 13, failure_mode 11, worked_design 9 — ERC 검증된 골든 픽스처 5종을 worked_design으로 편입, selection_guidance 8). **testprompt.md 18보드 A/B 실측**(동일 모델·하네스, 지식만 34→96): 가시 드래프트 17/18→18/18, 평균 KiCad 위반 185→139(−25%), 개선 11보드/악화 6보드(보드당 1회 실행이라 개별 수치는 노이즈, 합계는 방향성 유효). llama-index는 코퍼스가 수백~수천 규모로 커진 뒤 하이브리드(FTS+벡터) 시점에 도입 — 현 규모 판단은 Claude·Codex 일치. 반복 검증된 지식의 결정론 규칙 승격(Tier 2→코드)은 진행 중인 패턴(ensure_bus_pullups, complete_known_device_pins 등)이 그 사례다.
...
**블록 분해 구현 후 궤적 (v1→v6, 2026-08-07)**: 블록 분해를 구현하고 6회 라이브 반복으로 수렴 가드를 쌓은 결과 — v6 기준 **구조 계층은 완주**한다: 9블록 계획(POWER 5부품/MCU+SWD 헤더/드라이버×4/엔코더×4/CAN/서보), 64핀 MCU 블록 합성 성공(압축 핀 테이블 [num,name,type]로 출력 예산 확보), 수리 루프가 컨텍스트 안에서 동작하며 파괴적 연산 게이트(미지 lib_id 거부, 문제 무관 ref 거부)가 조작 피해를 차단. **남은 실패는 품질 계층**: 부품 선정 판단 오류(배터리 입력 보드에 AC/DC 컨버터 선택, "Bulk Capacitor" 검색→정전용량 버튼), 핀 이름의 '/'를 네트명에 복사, 수리 전략의 반복 정체 — 7B의 설계 판단 한계 + 도메인 가드(전원 도메인 후보 필터, 네트명 새니타이저) 추가 여지. ERC-0 보드 완주는 모델 업그레이드 또는 가드 확충 이후의 목표로 남긴다.
...
- **Phase 5 — 통합 검증 및 MVP 완료** 🟡 진행 중 (2026-08-07): ✅ 골든 5종 전체가 결정론 픽스처로 자체 ERC·KiCad ERC 0건+커넥티비티 라운드트립 통과(`goldens.py`, 회귀 게이트 `tests/test_golden_circuits.py`), ✅ 승인 감사 로그·리비전 불변성·결정론 재현(§12 기준, `audit.py`), ✅ webapp 최종 승인 엔드포인트. **남은 것**: 골든 2~5의 프롬프트→생성 라이브 경로 검증(= (a) 품질 가드 트랙 — 결정론 픽스처가 정답 기준선 역할), 승인 UI 완성도.
