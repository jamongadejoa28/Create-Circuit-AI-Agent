...
- **비전 파이프라인 아키텍처 결정**: 객체 탐지 모델(YOLO 계열)은 표/그림 영역의 **위치만** 찾는 detector라 JSON 변환에는 별도의 표 구조 인식+판독 단계가 어차피 필요하고, PyTorch 학습 스택은 §4의 "MVP에서 PyTorch 미사용" 원칙 및 8GB VRAM 제약과 충돌한다. 남은 처리 대상이 ~10개 표 수준이라 학습 파이프라인 구축은 비용 대비 과잉이다. **범위가 power electronics(Erickson Tier C — 수식·표가 전부 래스터 이미지)로 확장될 때에만** 배치 자동화가 정당화되며, 그때의 권장 구성은 기존 llama.cpp 인프라 위에 멀티모달 GGUF(예: Qwen2.5-VL-7B 양자화, 8GB VRAM에 적재 가능)를 올려 "렌더링→판독→JSON→검수" 루프를 오프라인 배치로 돌리는 것 — 텍스트 LLM과 동시 적재는 불가하므로 추출 배치는 생성 파이프라인과 별도 세션으로 운용한다. YOLO+PyTorch는 페이지 수천 장 규모에서 표 영역 탐지가 병목이 될 때만 재검토.
