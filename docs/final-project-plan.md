...
> **갱신 (2026-08-07)**: 공식 라이브러리 저장소를 로컬 클론해 소스를 전환했다. 심볼은 `kicad-symbols` 클론(태그 10.0.5 고정, 저장소 원본은 심볼당 1파일 `*.kicad_symdir` 레이아웃 — 설치본 단일 파일은 패키징 산출물)에서 읽으며 네이티브 ext4라 /mnt/c 대비 IO ~6배 빠르고 provenance가 git 태그로 명확해졌다. 풋프린트는 `kicad-footprints` 클론 15,448개를 같은 인덱스에 편입 — §8.2의 "풋프린트 핀 수 불일치"가 **핀 번호↔패드 번호 정합 검사**로 구현되었고, 비거나 잘못된 풋프린트는 심볼 기본값→`ki_fp_filters` glob 매칭 순으로 결정론 자동 배정된다(실전 첫 적발: 골든 픽스처에 지어냈던 존재하지 않는 풋프린트명). master 추종은 스탬프가 올라가 10.0.5 CLI가 못 읽게 될 위험이 있어 태그 고정을 유지한다.
...
**Tier B/C 처리 시점과 방법 (2026-08-07 실측으로 확정)**:

- **시점 — pull 방식**: Tier B 재추출은 별도 선행 단계가 아니라, 특정 값이 실제로 필요해진 시점에 그 표만 처리한다. MVP 필수 상수는 전부 Tier A로 이미 확보되어 있어 Tier B가 MVP 경로를 막지 않는다. 실제로 2차분에서 Tier B로 표기됐던 2개 표(Floyd Table 15-1 74xx 패밀리 비교, Sadiku Table 5.1 op-amp 파라미터)는 아래 비전 검증으로 당일 Tier A로 승격 완료 — 현재 지식 베이스에 Tier B 잔여분은 없다.
- **방법 — 실측 결과**: `page.find_tables()`(PyMuPDF)는 표 존재와 대략적 구조는 잡지만 다중 헤더 표에서 셀이 병합되어 나온다(Floyd Table 15-1에서 3개 패밀리 열이 한 셀로 붕괴 확인). 확실한 경로는 **페이지를 이미지로 렌더링(150dpi) → 멀티모달 모델이 표를 읽어 구조화 JSON 생성 → 사람 검수** — 이 루프를 실제로 돌려 두 표의 기존 값이 전부 정확함을 확인하고 승격했다.
- **비전 파이프라인 아키텍처 결정 (사전 데이터셋 구축 전용 — 서비스 런타임 아님)**: 이 파이프라인은 사용자가 제안한 대로 **오프라인 1회성 데이터셋 구축 도구**다 — 서비스 이용 시마다 도는 것이 아니므로 §4의 "MVP 런타임에서 PyTorch 미사용" 원칙과 충돌하지 않고, 추출 배치 시점에는 llama-server를 내리고 GPU를 독점 사용하면 된다. 단계 구성:
  1. **영역 검출**: born-digital PDF 7권은 PyMuPDF의 벡터 레이아웃 정보(블록/이미지/`find_tables` bbox)로 결정론적·무료로 해결 — 검출 모델이 필요 없다. 래스터 스캔북(Erickson Tier C)만 문서 레이아웃 검출 모델이 필요하며, 이때도 자체 학습이 아니라 **사전학습 가중치**(DocLayNet 계열 YOLO 문서 레이아웃 모델)를 그대로 쓴다 — 8GB GPU에 충분.
  2. **판독**: 영역 이미지 → 구조화 JSON. 로컬 멀티모달 VLM은 쓰지 않는다(사용자 결정 2026-08-07 — 7B급 양자화 VLM의 표 판독 신뢰도는 검증 안 된 추가 리스크). 대신 두 경로:
     - **1차 (큐레이션 세션)**: 개발을 진행하는 AI 어시스턴트(Claude)가 렌더링된 페이지 이미지를 직접 판독 — Tier B 표 2개를 이 방식으로 당일 검증·승격한 실적이 있으며, 추가 인프라가 전혀 필요 없다. 큐레이션 규모(수십~수백 항목)에서는 이것으로 충분.
     - **2차 (대량 배치, 어시스턴트 개입 없이 돌려야 할 때)**: 전용 OCR 스택 추가 — 텍스트는 PaddleOCR/Tesseract, 표 구조는 PP-Structure(PaddleOCR 표 인식) 또는 Table Transformer 사전학습 모델. 생성형 모델이 아니라 인식 전용 모델이므로 환각 없이 결정론에 가깝고, 4060 8GB 또는 CPU에서 무리 없이 동작. YOLO(영역 검출)와의 역할 분담은 그대로: 검출→OCR 판독→스키마 검증 JSON.
  3. **검수**: 사람 1회 검수(§6.2 원칙 유지) 후 `data/knowledge/`에 편입.
  4. **저장·벡터화**: SQLite 저장 + 임베딩 벡터화 — §4에 예약해둔 PyTorch 임베딩 트랙이 여기 연결되며, `search_knowledge`를 FTS 키워드 매칭에서 시맨틱 검색으로 업그레이드하는 경로(한/영 혼용 질의에 유리).
  - **발동 조건**: 현재 큐레이션 범위(34개 엔트리)는 수동+비전 검증으로 이미 처리 완료라 미가동. worked example 전수 추출 같은 대량 물결이나 Erickson 편입, 또는 시맨틱 검색 업그레이드가 필요해지는 시점에 구축한다. (2026-08-07 실측: 이 루프의 수동 원형 — 렌더링→판독→JSON 대조→검수 — 으로 Tier B 표 2개를 당일 Tier A로 승격했음.)
...
- **Phase 3 — 회로 계층 + MCU ERC 확장 (MVP 필수)** ✅(계층/Bus 제외): §8.2 확장 ERC 규칙(power_rails_shorted·decoupling_missing·i2c_pullup_missing·footprint_missing, unconnected_pin은 KiCad와 동일하게 error로 격상), 디커플링 배치 규칙(캡을 대상 IC 옆에 자동 배치), §7.5 라벨 밀집 완화 휴리스틱(입력좌/IC중앙/출력우 열 + 전원 상단·GND 하단 행) 완료. **종료 조건 달성: ESP32-WROOM-32 최소회로(디커플링+EN 풀업+IO0 스트랩+I2C 풀업+미사용 핀 NC 29개)가 결정론적 파이프라인만으로 KiCad ERC 0건 통과.** 계층 시트/Bus IR은 골든 회로 5종에 불필요하므로 Phase 6으로 이동(§9 Phase 6에 이미 명시됨).
